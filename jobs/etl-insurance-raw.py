...
def extract_raw_data_from_s3(s3_path):
    ...
    print("Extracting raw data from:", file_url)
    df = pd.read_csv(file_url)
    print("Successfully extracted CSV with", len(df), "rows and", len(df.columns), "columns")
    return df

def transform_raw_data(df):
    """Transform: Adds a timestamp column to track ingestion time."""
    df["timestamp_raw"] = datetime.now()  # Add current timestamp
    print("Transformation applied: Added 'timestamp_raw' column")
    return df

def load_raw_data_to_rds(df, host, port, database, username, password, table_name):
...
if __name__ == "__main__":
    ...
    df = extract_raw_data_from_s3(s3_path)

    # Transform: Add timestamp column
    df = transform_raw_data(df)

    # Load: Write transformed data to RDS
    load_raw_data_to_rds(df, host, port, database, username, password, table_name)
